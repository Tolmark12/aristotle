...
/* eLearningBrothers interface to SCORM */
var scorm = pipwerks.SCORM;
...
/* -------------------------------------------------------------------------
   elbScorm.GetUserName()
   Returns:    string (Kingsley, James)
---------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
   elbScorm.GetResumeData()
   Returns:    json ({key:var, key:var})
---------------------------------------------------------------------------- */

elbScorm.GetResumeData = function () {
	var resume_data = scorm.get('cmi.suspend_data')
	return (resume_data !="")? JSON.parse(resume_data) : null;
};

/* -------------------------------------------------------------------------
   elbScorm.SetResumeData()
   Parameters: module_state (json)
   Returns:    boolean (success ? true : false)
---------------------------------------------------------------------------- */

elbScorm.SetResumeData = function (module_state) {
	var suspend_data = JSON.stringify(module_state)
	var set_success = scorm.set('cmi.suspend_data', suspend_data)
	var save_success = scorm.save()
	return (set_success && save_success)
};
...
elbScorm.SetComplete = function () {
	...
	if (lmsConnected) {
		...
		if (success) {
			...
			elbScorm.handleError("Error: Course could not be set to complete!");
		}
		//If the course isn't connected to the LMS for some reason...
	} else {
		//alert the user and close the course window
		elbScorm.handleError("Error: Course is not connected to the LMS");
	}
	return success;
};
...
elbScorm.initCourse = function () {
	...
	if (lmsConnected) {
		...
		if (completionstatus == "completed" || completionstatus == "passed") {
			...
			elbScorm.handleError("You have already completed this course. You do not need to continue.");
		}
		//If the course couldn't connect to the LMS for some reason...
	} else {
		//... let's alert the user then close the window.
		elbScorm.handleError("Error: Course could not connect with the LMS");
	}
	return lmsConnected;
}


elbScorm.handleError = function (msg) {
	...
	if (pipwerks.debug.isActive) {
		...
		if (!!document.getElementById("scormErrors")) {
			document.getElementById("scormErrors").innerHTML = msg
		}
